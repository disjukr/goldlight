# 2d examples

Examples in this directory focus on the built-in 2D scene model.

- [`basic_rect`](./basic_rect): animated rectangles rendered by the Rust runtime
- [`fills`](./fills): layered filled path composition
- [`gradients`](./gradients): linear, radial, conical, and sweep gradients on filled paths
- [`strokes`](./strokes): stroke join and cap examples
- [`text_modes`](./text_modes): direct mask, SDF, and path-fallback text in one scene
- [`text_direct_mask`](./text_direct_mask): direct-mask glyph atlas text rendering
- [`text_sdf`](./text_sdf): signed-distance-field glyph rendering
- [`text_glyph_clusters`](./text_glyph_clusters): glyph-cluster visualization from shaped text
- [`text_on_path`](./text_on_path): path-fallback text arranged along guide curves
