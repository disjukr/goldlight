...
Windows-native BYOW example that mounts a real React scene through
`@disjukr/goldlight/react/reconciler` and renders the vendored Stanford Bunny PLY mesh on a native
WebGPU surface.
...
- normal TSX authoring against the `@disjukr/goldlight/react/reconciler` React-runtime JSX surface,
  using the same primitive contract as the snapshot path plus the exported convenience components
...
- reusable `@disjukr/goldlight/geometry` and `@disjukr/goldlight/math` helpers for generated
  normals, mesh bounds, and Euler-authored light rotation
...
```sh
deno task compile examples/byow/react_bunny_demo
```

This writes a debug build next to the entrypoint as
`examples/byow/react_bunny_demo/byow_react_bunny_demo.exe` by default. The local demo resources
...
```sh
deno task compile examples/byow/react_bunny_demo --release
```
...
```sh
deno task compile examples/byow/react_bunny_demo --output tmp/byow_react_bunny_custom.exe
```
