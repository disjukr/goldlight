...
`@disjukr/goldlight/drawing` does not currently draw images or textures, so a `g2d-scene` cannot yet
embed the output of a `g3d-scene`.
...
- `@disjukr/goldlight/react/reconciler` `g2d-scene` authoring inside `g3d-scene`
- offscreen `@disjukr/goldlight/drawing` rendering feeding a runtime texture
