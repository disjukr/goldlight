...
This directory hosts small ports of Skia `gm/` samples into `engine/drawing`.

Each GM lives in its own subdirectory and should usually provide:

- `render.ts`: Dawn/`@disjukr/goldlight/drawing` rendering path
...
- `3d`: blocked, out of scope for the 2D `engine/drawing` package
- `aaa`: done
- `aaclip`: pending
