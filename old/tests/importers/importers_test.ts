...
} from '@disjukr/goldlight/importers';

const textEncoder = new TextEncoder();
const fixturesRoot = join(Deno.cwd(), 'tests', 'fixtures');
...
Deno.test('listExternalGltfResourceUris normalizes relative URIs against URL and file bases', () => {
  ...
  assertEquals(
    listExternalGltfResourceUris(json, {
      baseUri: join(fixturesRoot, 'scene.gltf'),
    }),
    [
      join(fixturesRoot, 'geometry.bin'),
      join(fixturesRoot, 'textures', 'albedo.png'),
    ],
  );
