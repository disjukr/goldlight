...
Deno.test('collectRendererCapabilityIssues accepts forward lit baseColor textures with normals, uvs, and resident textures', () => {
  const residency = createRuntimeResidency();
  let scene = createSceneIr('scene');
  scene = appendLight(scene, {
    id: 'light-directional',
...
Deno.test('collectRendererCapabilityIssues rejects lit materials without lights or normals', () => {
  ...
  assertEquals(issues, [
    ...
    },
    {
      ...
      message:
        'renderer "forward" cannot sample baseColor textures on lit node "mesh-node" because mesh "mesh-0" is missing TEXCOORD_0',
    },
  ]);
});
